...
| **2** macOS clock | `08ce108` + this commit | 🟡 nearly met. Offline: the replay over this Mac's own timekeeper log (1,469 learned, 605 holds, 2,072 carried syncs, 2,077 intervals), time zones against `zoneinfo`, and `ak820 clock` read-only with a fail-closed refusal. Live (21:35): 6/6 captures decode byte-identically through the pinned C, Rust and `ak820ctl` reads agree within 0.7 ms, and 648,672 hours × 3 zones match the C's `localtime()`. Owed: the owner's LCD check; a system zone change moves to 4b |
