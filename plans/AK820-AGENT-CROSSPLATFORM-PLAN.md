...
| **CPU, media agent, while Music plays** | ✅ **After, the Rust daemon (2026-09-16 21:08–21:28, same method and conditions): ~0.2% of one core**: 0.37–0.46 CPU-s self per 240 s, 0 in children, 0.04 in the perl helper, no knock-on in `trustd`, `launchservicesd`, `tccd` or `loginwindow`. **Before:** ⚠️ **Re-taken 2026-09-16 19:41–20:01, just before 4a** (a quieter machine, but high-performance screen sharing was encoding the whole time, in run and pause windows alike): **45.5 / 57.7 / 54.2 CPU-s per 240 s in the agent's children, mean 52.5 = 21.9% of one core**. The run-minus-pause daemon deltas repeated across windows: `trustd` +9.5–10.2, `launchservicesd` +8.7–9.4, `tccd` +8.1–9.4, `loginwindow` +7.2–7.7 CPU-s per 240 s, **about 14% of a core more**, so **~36% of one core in all**. Window 1 carried an XProtect and `fseventsd` burst, and `backupd` and `VBCSCompiler` moved at random. The first "before": **47.4 CPU-s per 240 s = 19.7% of one core**, in the agent's own children alone (50.6 / 38.4 / 53.2 s over three windows; kernel child accounting, exact) — plus **~20–30 s** more in system daemons that only move while it runs: `tccd` 5.6–12.0, `launchservicesd` 6.9–8.8, `trustd` up to 9.8, `runningboardd` ~5, `launchd` ~2.6 above its paused level. **Roughly 30% of one core, 2.5% of this 12-core Mac, around the clock while music plays** **[measured 2026-09-16]** | **≤ 1% of one core** while playing. The event-driven helper it replaces the polling with measured **0.76 CPU-s in 98 min** (the sibling's perl helper, same Mac, same afternoon, music playing for part of it) |
