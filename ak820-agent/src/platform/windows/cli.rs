...
use super::device::{self, Device, Queue};
use super::path;
use crate::flash;
...
fn health_cmd(flags: &[&str]) -> Result<(), String> {
    ...
    if json {
        println!(
            "{}",
            health::render_json_with_vitals(health::render_json_with_crash(health::render_json(&p1, p2.as_ref(), p3.as_ref(), p4.as_ref().map(|(b, r)| (b, r))), crash.as_ref()), p6.as_ref())
        );
    } else {
...
fn retire_python(clock: bool, dir: &std::path::Path) -> Result<(), String> {
    use super::{instance, process, task};
    use std::time::Duration;
...
fn uninstall() -> Result<(), String> {
    ...
    println!("binaries and logs left in {}", super::default_dir().display());
    println!(
...
fn probe() -> Result<(), String> {
    ...
    let sessions = super::smtc::poll_once().map_err(|e| format!("SMTC unavailable: {e}"))?;
    ...
    for (command, body) in smtc::reports(&snap) {
        let frame = crate::proto::frame(crate::proto::Channel::Text, command, &body);
