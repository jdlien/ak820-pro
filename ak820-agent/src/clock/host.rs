...
//!
//! The real hosts live under `crate::platform` (`SystemHost` for each OS), with
//! the tests that sweep them against their C runtime's `localtime`. This file
//! holds the trait, the pure pieces, and the scripted fake.
#[cfg(test)]
...
pub trait Host {
    ...
    /// second the OS cannot place in a calendar, and the C dereferences the
...
/// `localtime`; also what a reader can check a real host against by hand.
